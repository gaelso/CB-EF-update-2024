...
luvs_prelim_rua <- map(path_rua, readxl::read_xlsx, sheet = "F3", col_types = "text") |> 
  list_rbind() |>
  select(
    cluster_no = "1. Cluster Number",
    plot_no    = "2. Plot Number",                                                    
...
luvs_init_rua <- luvs_prelim_rua |> 
  select(luvs_cluster_no, luvs_plot_no, starts_with("luvs_luvs")) |>
  pivot_longer(
    cols = -c(luvs_cluster_no, luvs_plot_no), 
    names_to = c("luvs_luvs_no", ".value"), 
    names_pattern = "luvs_luvs(.)_(.*)"
    ) |>
  filter(plot_share != "NA") |>
  select(-no) |>
